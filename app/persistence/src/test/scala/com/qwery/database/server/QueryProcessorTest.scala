...
class QueryProcessorTest extends AnyFunSpec {
  ...
  describe(classOf[QueryProcessor].getName) {
    ...
    it(s"should insert $insertCount rows into a table") {
      ...
      val newQuote = () => Map("symbol" -> randomSymbol, "exchange" -> randomExchange, "lastSale" -> randomPrice, "lastTradeTime" -> randomDate)
      ...
      responses.sortBy(-_.responseTime).take(20).zipWithIndex foreach { case (response, n) => logger.info(f"insert[$n%03d]: $response") }
    }
