...
import com.qwery.database.{Column, ColumnMetadata, Row, RowTuple, TableFile}
  ...
  describe(classOf[ColumnOrientedFileBlockDevice].getSimpleName) {
    ...
    it("should read/write columnar data") {
      ...
      ColumnOrientedFileBlockDevice(columns, file) use { implicit device =>
        ...
        val buf0 = RowTuple("symbol" -> "AAPL", "exchange" -> "NASDAQ", "lastSale" -> 99.98, "lastSaleTime" -> new Date).toRowBuffer
        device.writeRowAsBinary(rowID = 0, buf0)

        // write a record to the table
        val buf1 = RowTuple("symbol" -> "GE", "exchange" -> "NYSE", "lastSale" -> 56.45, "lastSaleTime" -> new Date).toRowBuffer
