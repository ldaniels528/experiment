...
class Tabular() {
  ...
  def transformMaps[A](values: Seq[Map[String, A]]): Seq[String] = {
    if (values.isEmpty) Nil
      ...
      // get the headers, data rows, and column widths
      val headers = values.head.keys.toSeq
  ...
  protected def invokeMethod[A](v: A, f: String): AnyRef = {
  ...
  /**
    ...
    */
  protected def columnWidths(headers: Seq[String], rows: Seq[Map[String, String]]): Seq[Int] = {
  ...
  /**
    ...
    */
  protected def unwantedFields(s: String): Boolean = Set("$outer").contains(s)
