...
import com.qwery.platform.spark.SparkFilesystem._
...
case class SparkFilesystem(path: String) extends SparkInvokable {
  override def execute(input: Option[DataFrame])(implicit rc: SparkQweryContext): Option[DataFrame] = Option(getFilesAsDf(path))
}
...
object SparkFilesystem {
  ...
  /**
    ...
    */
  def getFilesAsDf(path: String)(implicit rc: SparkQweryContext): DataFrame = {
