...
import org.apache.spark.sql.{ColumnName, DataFrame, SaveMode, Column => SparkColumn}
...
trait EmbeddedSparkCompiler {
  ...
  def compile(condition: Condition)(implicit rc: EmbeddedSparkContext): SparkColumn = condition.compile
  ...
  /**
    ...
    */
  @throws[IllegalArgumentException]
  def compile(expression: Expression)(implicit rc: EmbeddedSparkContext): SparkColumn = expression.compile
  ...
  /**
    ...
    */
  @throws[IllegalArgumentException]
  def compile(statement: Invokable)(implicit rc: EmbeddedSparkContext): SparkInvokable = statement.compile
  ...
  /**
    ...
    */
  @throws[IllegalArgumentException]
  def compileAndRun(fileName: String, statement: Invokable, args: Seq[String])(implicit rc: EmbeddedSparkContext): Unit = {
...
object EmbeddedSparkCompiler {
  ...
  /**
    ...
    * @param rc          the implicit [[EmbeddedSparkContext]]
    */
  def write(source: DataFrame, destination: Location, append: Boolean)(implicit rc: EmbeddedSparkContext): Unit =
  ...
  /**
    ...
    * @param rc          the implicit [[EmbeddedSparkContext]]
    */
  def write(source: DataFrame, destination: TableLike, append: Boolean)(implicit rc: EmbeddedSparkContext): Unit = destination match {
  ...
  case class ReadRowSetByReference(name: String) extends SparkInvokable with Aliasable {
    override def execute(input: Option[DataFrame])(implicit rc: EmbeddedSparkContext): Option[DataFrame] = rc.getDataSet(name, alias)
  }
  ...
  object Implicits {
    ...
    final implicit class ConditionCompiler(val condition: Condition) extends AnyVal {
      def compile(implicit rc: EmbeddedSparkContext): SparkColumn = {
        import com.qwery.models.expressions._
        condition match {
    ...
    final implicit class ExpressionCompiler(val expression: Expression) extends AnyVal {
      ...
      private def compileCase(model: Case)(implicit rc: EmbeddedSparkContext): SparkColumn = {
        import org.apache.spark.sql.functions._
    ...
    final implicit class InvokableCompiler(val invokable: Invokable) extends AnyVal {
      ...
      def compile(implicit rc: EmbeddedSparkContext): SparkInvokable = invokable match {
        ...
        case SetLocalVariable(name, expression) => SparkSetLocalVariable(name, value = (_: EmbeddedSparkContext) => expression.asAny)
      ...
      def resolveColumns(implicit rc: EmbeddedSparkContext): List[Column] = invokable match {
      ...
      /**
        ...
        */
      private def incorporateSources(path: String)(implicit rc: EmbeddedSparkContext): SparkInvokable = {
    ...
    final implicit class JoinEnrichment(val join: Join) extends AnyVal {
      @inline def compile(implicit rc: EmbeddedSparkContext): SparkJoin =
    ...
    final implicit class LocationEnrichment(val location: Location) extends AnyVal {
      ...
      /**
        ...
        */
      @inline def resolveColumns(implicit rc: EmbeddedSparkContext): List[Column] = rc.getTableOrView(location).resolveColumns

      /**
        * Attempts to retrieve the desired data frame for this [[Location]]
        * @return the [[Table table]]
        */
      @inline def getQuery(implicit rc: EmbeddedSparkContext): Option[DataFrame] = location match {
    ...
    final implicit class TableLikeEnrichment(val tableLike: TableLike) extends AnyVal {
      ...
      /**
        ...
        * @return the collection of [[Column columns]]
        */
      @inline def resolveColumns(implicit rc: EmbeddedSparkContext): List[Column] = {
        tableLike match {
