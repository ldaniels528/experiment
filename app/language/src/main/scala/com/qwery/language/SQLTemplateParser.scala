...
class SQLTemplateParser(stream: TokenStream) extends ExpressionParser with SQLLanguageParser {
    ...
    val source = stream match {
      ...
      case ts if ts nextIf "VALUES" =>
        ts match {
          ...
          case ts1 if ts1 nextIf "$" => ts1.die("Scalar variable references are not compatible with row sets")
          case ts1 =>
  ...
  private def extractTableClause(name: String): Try[SQLTemplateParams] = Try {
    ...
    while (isHiveOrAthena) {
      stream match {
        case ts if ts nextIf "COMMENT" => template += SQLTemplateParams(ts, "%a:description")
